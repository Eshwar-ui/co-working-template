...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const sidebar = document.querySelector('#sidebar') || document.querySelector('#dashboardSidebar');
  ...
  const notifBtn = document.querySelector('#notifBtn') || document.querySelector('.notification-toggle');
  const notifDropdown = document.querySelector('#notifDropdown') || document.querySelector('.notification-dropdown');
  if (notifBtn && notifDropdown) {
    notifBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      notifDropdown.classList.toggle('hidden');
      // Close profile dropdown
      const profileDD = document.querySelector('#profileDropdown') || document.querySelector('.profile-dropdown');
      if (profileDD) profileDD.classList.add('hidden');
    });
  }

  // ---------- Profile Dropdown ----------
  const profileBtn = document.querySelector('#profileBtn') || document.querySelector('.profile-toggle');
  const profileDropdown = document.querySelector('#profileDropdown') || document.querySelector('.profile-dropdown');
  ...
  if (darkToggle) {
    ...
    darkToggle.addEventListener('click', function () {
      ...
      const icon = darkToggle.querySelector('i');
      if (icon) {
        if (currentlyDark) { icon.classList.replace('fa-moon', 'fa-sun'); } 
    ...
    if (link.textContent.trim().toLowerCase() === 'logout') {
      link.addEventListener('click', (e) => {
        ...
        if (confirm('Are you sure you want to logout?')) {
          ...
          const isDeep = window.location.pathname.includes('/admin/') || window.location.pathname.includes('/dashboard/');
  ...
  document.querySelectorAll('.booking-slot:not(.unavailable)').forEach(slot => {
    slot.addEventListener('click', function () {
      document.querySelectorAll('.booking-slot').forEach(s => s.classList.remove('selected', 'border-primary', 'bg-primary/5'));
  ...
  if (revenueChartEl && typeof ApexCharts !== 'undefined') {
    ...
    const options = {
      series: [{
        name: 'Revenue',
        data: [45000, 52000, 38000, 65000, 48000, 56000, 70000, 62000, 85000, 78000, 92000, 105000]
      }],
      chart: {
        type: 'area',
        height: 288,
      ...
      darkToggleBtn.addEventListener('click', () => {
        setTimeout(() => {
          ...
          chart.updateOptions({
            ...
            grid: { borderColor: dark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)' }
          });
        }, 50);
      });
    }
  ...
  window.showToast = function (message, type = 'success') {
    ...
    const colors = {
      success: 'bg-primary', error: 'bg-red-500',
      warning: 'bg-secondary', info: 'bg-primary'
    };
